...
if [ -v 1 ] || [ -v 2 ]
then
   # Check to see if we are encrypting and the given file exists
   if [ "$1" == "-e" ] && [ -f "$2" ]
   then
      # Encrypts the file
      ${OPENSSL} enc -aes-256-cbc -md sha512 -pbkdf2 -iter 100000 -salt -in $2 -out $2.dat
      # Safely deletes the original file
      ${SRM} $2
      echo "${2} encrypted to ${2}.dat"
   # Check to see if we are decrypting the file and the given file exists   
   elif [ "$1" == "-d" ] && [ -f "${2}.dat" ]
   then
       # Decrypts the file
  ...
  # If no argument values are given the following is displayed.
